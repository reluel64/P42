...
void* liballoc_alloc(size_t len)
{
    vmmgr_alloc(0, len * PAGE_SIZE, VMM_ATTR_WRITABLE);
}
 ...
 */
int liballoc_free(void*ptr,size_t len)
{
    return(vmmgr_free(ptr, len * PAGE_SIZE));
}

int liballoc_lock()
{
    spinlock_lock_interrupt(&liblock);
    return(0);
}
...
int liballoc_unlock()
{
    spinlock_unlock_interrupt(&liblock);
    return(0);
}