...
static inline int vm_split_block
(
...
int vm_init(void)
{
    ...
    memset(fh_hi, 0, VMM_SLOT_SIZE);
    memset(fh_lo, 0, VMM_SLOT_SIZE);
    memset(ah,    0, VMM_SLOT_SIZE);
    ...
    kernel_ctx.free_per_slot = (PAGE_SIZE - sizeof(vm_free_mem_hdr_t)) /
                                              sizeof(vm_free_mem_t);

    /* How many reserved entries can we store per page */
    kernel_ctx.rsrvd_per_slot = (PAGE_SIZE - sizeof(vm_rsrvd_mem_hdr_t)) /
                                                sizeof(vm_rsrvd_mem_t);
...
/*
 * vm_alloc_tracking - allocate tracking memory
...
static virt_addr_t vm_alloc_tracking(vm_ctx_t *ctx)
{
    vm_free_mem_hdr_t *fh    = NULL;
...
)
{
    ...
    vm_rsrvd_mem_hdr_t *rh        = NULL;
    vm_rsrvd_mem_t     *rdesc     = NULL;
    vm_rsrvd_mem_t     *candidate = NULL;
    vm_rsrvd_mem_t      new_rsrvd = {0};
    uint8_t                done      = 0;
    ...
    while(rn)
    {
        ...
        if(rh->avail > 0)
        {
            for(uint16_t i = 0; i < ctx->rsrvd_per_slot; i++)
            {
                ...
                /* check if we are trying to reserve again 
                 * TODO: we should also check for sub-ranges
                 */
                 
                else if ((rdesc->base == virt) && 
                         (rdesc->length == len) && 
                         rdesc->type == type)
                {
                    return(-1);
        ...
        /* We don't have available slots so we must
         * acquire a new slot and prepare it
         */ 
        if(next_rn == NULL)
        {
            next_rn = (list_node_t*)vm_alloc_tracking(ctx);

            if(next_rn != NULL)
            {
                rh = (vm_rsrvd_mem_hdr_t*)next_rn;
...
static int vm_merge_free_block
(
    vm_ctx_t *ctx, 
    virt_addr_t virt,
    virt_size_t len
)
{
    ...
    vm_free_mem_hdr_t *fh      = NULL;
    vm_free_mem_t     *fdesc   = NULL;
    vm_free_mem_t     *to_merge = NULL;
    vm_free_mem_hdr_t *to_merge_hdr = NULL;
    int                   merged  = 0;
...
/* vm_put_free_mem - place a free memory block in the free memory list 
 * This routine is similar in implementation with vm_add_reserved
 * */

static int vm_put_free_mem
(
    vm_ctx_t *ctx, 
    virt_addr_t virt,
    virt_size_t len
)
{
    ...
    while(fn)
    {
        ...
        if(next_fn == NULL)
        {
            next_fn = (list_node_t*)vm_alloc_tracking(ctx);

            if(next_fn != NULL)
            {
                fh = (vm_free_mem_hdr_t*)next_fn;
                fh->avail = ctx->free_per_slot;

                linked_list_add_tail(&ctx->free_mem, &fh->node);

                vm_add_reserved(ctx, (virt_addr_t)next_fn, 
                                        VMM_SLOT_SIZE, 
                                        VMM_RES_FREE);
            }
        }
...
)
{
    ...
    if(vm_split_block(&from_slot->base,
                         &from_slot->length,
                         ret_addr, 
                         len, 
                         &rem_virt,
